...
class MetVap():
    ...
    def set_pT_robust(self, p, T):
        if T<190.0:
            self.states.update(CoolProp.QT_INPUTS, 1.0, T) #pretpostavka - zasiceno stanje
            p_sat = self.get_p()
        else:
            p_sat = 9999999.0
        if (abs(p_sat-p) > 2): #ipak nije zasiceno
            self.states.update(CoolProp.PT_INPUTS, p, T)



    def set_T_sat(self, T):
        self.states.update(CoolProp.QT_INPUTS, 1.0, T)
        #K
    def set_p_sat(self, p):
        self.states.update(CoolProp.PQ_INPUTS, p, 1.0)
    def set_HD_molar(self, Hmolar, Dmolar):
        self.states.update(CoolProp.DmolarHmolar_INPUTS, Dmolar, Hmolar)
        ph = self.get_phase()
        assert ((ph == 5) or (ph == 2)), 'phase: ' + str(ph) + ' D: ' + str(Dmolar) + ' H: ' + str(Hmolar)
    def set_Hmolar_p(self, Hmolar, p):
        Hmolar_Jmol = Hmolar / 1000.0
        self.states.update(CoolProp.HmolarP_INPUTS, Hmolar_Jmol, p) #pazi - hmolar - J/kmol, Hmolar_Jmol - J/mol)

    def set_pSmol(self, p, S_molar):
        self.states.update(CoolProp.PSmolar_INPUTS, p, S_molar)
        ph = self.get_phase()
        assert ((ph == 5) or (ph == 2)), 'phase: ' + str(ph) + ' p: ' + str(p) + ' S: ' + str(S_molar)

    def set_pHmol(self, p, H_molar):
        self.states.update(CoolProp.HmolarP_INPUTS, H_molar, p)
        ph = self.get_phase()
        assert ((ph == 5) or (ph == 2)), 'phase: ' + str(ph) + ' p: ' + str(p) + ' H: ' + str(H_molar)
    ...
    def get_mol_density(self):
        return self.states.rhomolar()
    ...
    def get_mol_inten(self):
        return self.states.umolar() #J/mol ili kJ/kmol
...
class MetVap_slow():
    ...
    def set_UD_molar(self, Umolar, Dmolar):
        self.states.update(CoolProp.DmolarUmolar_INPUTS, Dmolar, Umolar)
