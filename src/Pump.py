...
class LngPump:
    def __init__(self, eta, states):
        self.eta = eta
        self.states = states
        self.save = PumpSave(self.states)
    ...
    def calculate(self,T_in, p_in, p_out):
        ...
        met_liq.set_T(T_in)
        H_mol_in = met_liq.get_mol_enthalpy()
        self.states.H_mol_in = H_mol_in
        S_mol_in = met_liq.get_mol_entropy()
        self.states.S_mol_in = S_mol_in
        #izentropa
        met_liq.set_pSmol(p_out, S_mol_in)

        h_isentropic = met_liq.get_mol_enthalpy()
        ...
        met_liq.set_pHmol(p_out, H_mol_out)
        T_out = met_liq.get_T()
...
if __name__ == "__main__":
    ...
    p_in0 = 100000.0
    p_out0 = 600000.0
    sts = PumpStates(T_in0, T_out0, p_in0, p_out0)
    pmp = LngPump(eta, sts)
    pmp.calculate(T_in0, p_in0, p_out0)
